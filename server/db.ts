...
export async function insertLead(lead: InsertLead): Promise<number> {
  const db = await getDb();
  if (!db) throw new Error("[Database] Cannot insert lead: database not available");
...
export async function updateLeadStatus(
  ...
  const db = await getDb();
  if (!db) return;
  await db.update(leads).set({ status }).where(eq(leads.sessionId, sessionId));
}

export async function markLeadWebhookSent(id: number): Promise<void> {
  const db = await getDb();
  if (!db) return;
  await db.update(leads).set({ webhookSent: 1 }).where(eq(leads.id, id));
}
