...
var leadsRouter = router({
  ...
  submitInitial: publicProcedure.input(leadBaseSchema).mutation(async ({ input }) => {
    ...
    try {
      ...
      console.log(`[leads.submitInitial] \u2705 Webhooks conclu\xEDdos \u2014 Sheets: ${results.sheets} | BotConversa: ${results.botconversa}`);
    } catch (webhookErr) {
      console.error("[leads.submitInitial] \u274C Erro nos webhooks:", webhookErr instanceof Error ? webhookErr.message : String(webhookErr));
    }
    try {
      await notifyOwner({
        title: "\u{1F514} Novo lead (Incompleto) \u2014 Mora Care",
